...
class FirestoreStorage implements IStorage {
  async getCustomers() { if (!db) return []; const snap = await (db as any).collection('customers').orderBy('createdAt','desc').get(); return snap.docs.map((d:any)=>({ id: d.id, ...d.data() })); }
  async getCustomer(id: number) { if (!db) return undefined; const doc = await (db as any).collection('customers').doc(String(id)).get(); return doc.exists ? ({ id: doc.id, ...doc.data() } as any) : undefined; }
  async createCustomer(customer: InsertCustomer) { if (!db) throw new Error('Firestore not initialized'); const ref = await (db as any).collection('customers').add({ ...customer, createdAt: new Date().toISOString() }); const doc = await ref.get(); return { id: ref.id, ...doc.data() } as any; }
  async updateCustomer(id: number, customer: Partial<InsertCustomer>) { if (!db) return undefined; const ref = (db as any).collection('customers').doc(String(id)); await ref.set({ ...customer }, { merge: true }); const doc = await ref.get(); return { id: doc.id, ...doc.data() } as any; }
  async deleteCustomer(id: number) { if (!db) return false; await (db as any).collection('customers').doc(String(id)).delete(); return true; }

  async getInventoryMovements() { if (!db) return []; const snap = await (db as any).collection('inventoryMovements').orderBy('createdAt','desc').get(); return snap.docs.map((d:any)=>({ id: d.id, ...d.data() })); }
  async getInventoryMovement(id: number) { if (!db) return undefined; const doc = await (db as any).collection('inventoryMovements').doc(String(id)).get(); return doc.exists ? ({ id: doc.id, ...doc.data() } as any) : undefined; }
  async createInventoryMovement(movement: InsertInventoryMovement) { if (!db) throw new Error('Firestore not initialized'); const ref = await (db as any).collection('inventoryMovements').add({ ...movement, createdAt: new Date().toISOString() }); const doc = await ref.get(); return { id: ref.id, ...doc.data() } as any; }
  async getCurrentStock() { const movements = await this.getInventoryMovements(); if (movements.length === 0) return 0; return (movements[0] as any).balance ?? 0; }
  async updateInventoryMovement(id: number, movement: Partial<InsertInventoryMovement>) { if (!db) return undefined; const ref = (db as any).collection('inventoryMovements').doc(String(id)); await ref.set({ ...movement }, { merge: true }); const doc = await ref.get(); return { id: doc.id, ...doc.data() } as any; }
  async deleteInventoryMovement(id: number) { if (!db) return false; await (db as any).collection('inventoryMovements').doc(String(id)).delete(); return true; }

  async getSales() { if (!db) return []; const snap = await (db as any).collection('sales').orderBy('createdAt','desc').get(); return snap.docs.map((d:any)=>({ id: d.id, ...d.data() })); }
  async getSale(id: number) { if (!db) return undefined; const doc = await (db as any).collection('sales').doc(String(id)).get(); return doc.exists ? ({ id: doc.id, ...doc.data() } as any) : undefined; }
  async createSale(sale: InsertSale) { if (!db) throw new Error('Firestore not initialized'); const ref = await (db as any).collection('sales').add({ ...sale, createdAt: new Date().toISOString() }); const doc = await ref.get(); return { id: ref.id, ...doc.data() } as any; }
  async updateSale(id: number, sale: Partial<InsertSale>) { if (!db) return undefined; const ref = (db as any).collection('sales').doc(String(id)); await ref.set({ ...sale }, { merge: true }); const doc = await ref.get(); return { id: doc.id, ...doc.data() } as any; }
  async deleteSale(id: number) { if (!db) return false; await (db as any).collection('sales').doc(String(id)).delete(); return true; }

  async getExpenseCategories() { if (!db) return []; const snap = await (db as any).collection('expenseCategories').orderBy('createdAt','desc').get(); return snap.docs.map((d:any)=>({ id: d.id, ...d.data() })); }
  async getExpenseCategory(id: number) { if (!db) return undefined; const doc = await (db as any).collection('expenseCategories').doc(String(id)).get(); return doc.exists ? ({ id: doc.id, ...doc.data() } as any) : undefined; }
  async createExpenseCategory(category: InsertExpenseCategory) { if (!db) throw new Error('Firestore not initialized'); const ref = await (db as any).collection('expenseCategories').add({ ...category, createdAt: new Date().toISOString() }); const doc = await ref.get(); return { id: ref.id, ...doc.data() } as any; }
  async updateExpenseCategory(id: number, category: Partial<InsertExpenseCategory>) { if (!db) return undefined; const ref = (db as any).collection('expenseCategories').doc(String(id)); await ref.set({ ...category }, { merge: true }); const doc = await ref.get(); return { id: doc.id, ...doc.data() } as any; }
  async deleteExpenseCategory(id: number) { if (!db) return false; await (db as any).collection('expenseCategories').doc(String(id)).delete(); return true; }

  async getExpenses() { if (!db) return []; const snap = await (db as any).collection('expenses').orderBy('createdAt','desc').get(); return snap.docs.map((d:any)=>({ id: d.id, ...d.data() })); }
  async getExpense(id: number) { if (!db) return undefined; const doc = await (db as any).collection('expenses').doc(String(id)).get(); return doc.exists ? ({ id: doc.id, ...doc.data() } as any) : undefined; }
  async createExpense(expense: InsertExpense) { if (!db) throw new Error('Firestore not initialized'); const ref = await (db as any).collection('expenses').add({ ...expense, createdAt: new Date().toISOString() }); const doc = await ref.get(); return { id: ref.id, ...doc.data() } as any; }
  async updateExpense(id: number, expense: Partial<InsertExpense>) { if (!db) return undefined; const ref = (db as any).collection('expenses').doc(String(id)); await ref.set({ ...expense }, { merge: true }); const doc = await ref.get(); return { id: doc.id, ...doc.data() } as any; }
  async deleteExpense(id: number) { if (!db) return false; await (db as any).collection('expenses').doc(String(id)).delete(); return true; }

  async getIngredients() { if (!db) return []; const snap = await (db as any).collection('ingredients').orderBy('name').get(); return snap.docs.map((d:any)=>({ id: d.id, ...d.data() })); }
  async getIngredient(id: number) { if (!db) return undefined; const doc = await (db as any).collection('ingredients').doc(String(id)).get(); return doc.exists ? ({ id: doc.id, ...doc.data() } as any) : undefined; }
  async createIngredient(ingredient: InsertIngredient) { if (!db) throw new Error('Firestore not initialized'); const ref = await (db as any).collection('ingredients').add({ ...ingredient, createdAt: new Date().toISOString() }); const doc = await ref.get(); return { id: ref.id, ...doc.data() } as any; }
  async updateIngredient(id: number, ingredient: Partial<InsertIngredient>) { if (!db) return undefined; const ref = (db as any).collection('ingredients').doc(String(id)); await ref.set({ ...ingredient }, { merge: true }); const doc = await ref.get(); return { id: doc.id, ...doc.data() } as any; }
  async deleteIngredient(id: number) { if (!db) return false; await (db as any).collection('ingredients').doc(String(id)).delete(); return true; }

  async getSetting(key: string) { if (!db) return undefined; const doc = await (db as any).collection('settings').doc(key).get(); return doc.exists ? String((doc.data() as any).value) : undefined; }
  async setSetting(key: string, value: string) { if (!db) throw new Error('Firestore not initialized'); await (db as any).collection('settings').doc(key).set({ value }); }
}

const useFirestore = process.env.USE_FIRESTORE === '1' && initialized && db;

// Create underlying instances
const memInstance = new MemStorage();
const firestoreInstance = useFirestore ? (new FirestoreStorage() as IStorage) : null;

// Proxy: prefer Firestore but fall back to MemStorage on failures
export const storage: IStorage = new Proxy(memInstance as unknown as IStorage, {
  get(target, prop: string | symbol) {
    // If Firestore is available and implements the method, call it first
    const f = firestoreInstance && (firestoreInstance as any)[prop];
    const m = (target as any)[prop];
    ...
    if (typeof f === 'function') {
      return async (...args: any[]) => {
        try {
          ...
          // On any Firestore error, log and gracefully fall back to memory/file storage
          // eslint-disable-next-line no-console
          console.error(`Firestore operation ${String(prop)} failed, falling back to local storage:`, (err as any) && (err as any).message ? (err as any).message : err);
          return await m.apply(target, args);
        }
      };
    }

    // Otherwise return the memory-backed implementation
    return m;
  }
});